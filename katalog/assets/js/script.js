...
const detailproduct = new Map([
  [
    "BeAT",
    [
      "./assets/images/ahm-gaul-sideview-deluxe-black-7-01022023-085330.webp",
  ...
  [
    "KTM RC 200",
    [
      "./assets/images/rc 200.jpg",
      "Tips Perawatan Motor KTM RC 200",
      "Untuk menjaga performa dan memastikan umur panjang kendaraan:<br> 1. Baterai: Jaga baterai tetap terisi penuh, terutama jika motor tidak digunakan dalam waktu yang lama. Pertimbangkan penggunaan trickle charger untuk menjaga daya baterai.<br> 2. Gunakan Suku Cadang Asli: Ketika perlu mengganti suku cadang, gunakan suku cadang asli dari pabrikan. Hal ini memastikan kompatibilitas dan kinerja yang optimal.",
    ],
  ],
  [
    "KTM DUKE 200",
    [
      "./assets/images/duke 200.jpg",
      "Tips Perawatan Motor KTM DUKE 200",
      "Untuk menjaga performa dan memastikan umur panjang kendaraan:<br> 1. Tekanan Ban: Selalu cek tekanan ban sebelum berkendara. Ban dengan tekanan yang tepat memastikan keamanan, kenyamanan, dan efisiensi bahan bakar.<br> 2. Periksa dan Ganti Cairan: Pastikan semua cairan (oli mesin, cairan rem, cairan radiator) pada level yang tepat dan ganti sesuai rekomendasi pabrikan.",
    ],
  ],
  [
    "KTM 450 SX-F",
    [
      "./assets/images/sx-f.jpg",
      "Tips Perawatan Motor KTM 450 SX-F",
      "Untuk menjaga performa dan memastikan umur panjang kendaraan:<br> 1. Pemeriksaan Rem: Periksa ketebalan pad rem dan kondisi disk rem secara berkala. Ganti pad rem yang sudah menipis untuk menghindari kerusakan pada disk rem.<br> 2. Baterai: Jaga baterai tetap terisi penuh, terutama jika motor tidak digunakan dalam waktu yang lama. Pertimbangkan penggunaan trickle charger untuk menjaga daya baterai.",
    ],
  ],
  [
    "Vitpilen 401",
    [
      "./assets/images/vitpilen.jpg",
      "Tips Perawatan Motor Husqvarna Vitpilen 401",
      "Untuk menjaga performa dan memastikan umur panjang kendaraan:<br> 1. Pemeliharaan Rantai: Bersihkan dan lumasi rantai secara berkala. Penggunaan pelumas rantai yang sesuai dapat memperpanjang umur rantai dan meningkatkan kinerja berkendara.<br> 2. Kebersihan Motor: Cuci motor secara teratur untuk menghilangkan kotoran, pasir, atau garam yang bisa mengikis bagian logam dan cat. Gunakan sabun yang khusus dirancang untuk kendaraan dan hindari sabun cuci piring yang dapat merusak lapisan wax.",
    ],
  ],
  [
    "FC 250",
    [
      "./assets/images/fc 250.jpg",
...
document.getElementById("Honda").addEventListener("click", (event) => {
  if (document.getElementById("lineup-list").childNodes.length == 0) {
    ...
    Array.from(document.getElementById("lineup-list").childNodes).forEach(
      (node) => {
        console.log(node);
        node.remove();
      }
...
document.getElementById("cari-merk").addEventListener("input", function (e) {
  e.preventDefault();
  if (e.target.value) {
    // untuk mencari target yang di search, lalu menhapus yang tidak sesuai
    Array.from(document.getElementById("merk-list").childNodes).forEach(
      (node) => {
        node.remove();
      }
    );
    for (const merk of product.keys()) {
      // ini untuk menghilangkan case sensitif
    ...
    // apabila tidak melakukan search
    Array.from(document.getElementById("merk-list").childNodes).forEach(
      (node) => {
        node.remove();
      }
...
function capitalizeFirstLetterOfEachWord(input) {
  // fungsi untuk menvalidasi perkata atau menghilangkan case sensitif
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (clickedElement.classList.contains("merk")) {
      ...
      if (document.getElementById("lineup-list").childNodes.length == 0) {
        // ini kalau di pencet bagian nama motornya
...
document.getElementById("cari-lineup").addEventListener("input", function (e) {
  ...
  if (e.target.value) {
    ...
    for (const lineupList of product.values()) {
      // ini untuk menghilangkan case sensitif
